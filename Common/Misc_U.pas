...
Var xSL_Linhas_Debug       : TStringList ;
    SL_Inibicao_Reload_FN  : TStringList ;
    FN_Last_Log_Full       : String = '' ;
    Indice_Log             : LongInt = 0 ;
    Last_Log               : Cardinal = 0 ;
    First_Tick_Log         : Cardinal = 0 ;
    SL_Debug_Full          : TStringList ;
    Cnt_Debug              : LongInt = 0 ;
    Cnt_Debug_Full         : LongInt = 0 ;
    Cnt_Indice_Put_Status1 : LongInt = 0 ;
    Cnt_Indice_Put_Status2 : LongInt = 0 ;
    SB_Log_Status1         : TStatusBar ;
    SB_Log_Status2         : TStatusBar ;
    MISC_PN_STATUS         : TPanel = Nil ;
    Dir_Mon                : Array Of T_Supervisao_Diretorio ;
    Supervisor_Pasta       : Array Of TSupervisor_Pasta ;
    Dicionario_MM          : TStringList ;
    SL_Batch               : TStringList ;
    Buffer_Log_FFMPEG      : Array Of String ;
    FFMPEG                 : Tffmpeg ;
    DIR_BASE               : String = 'C:\CC\' ;
    DEF_BASE_PROGRAMAS_MASTER : String = 'C:\CC\' ;
    FLAG_FORM_SELOS_OK     : Boolean = FALSE ;
    FLAG_FORM_SPLASH_OK    : Boolean = FALSE ;
    Diretorio_Chyron2      : String = '\\Compix_Futebol\C\Remoto\' ;
    Diretorio_Inscriber    : String = '\\Compix_Futebol\C\Remoto\' ;
    FLAG_PRIMEIRO_RELOGIO_OK : Boolean = FALSE ;
    CONFIG_DIR_VIDEO       : String = '' ;
    CONFIG_DIR_VIDEO_HD    : String = 'C:\CC\HD\' ;
    CONFIG_DIR_VIDEO_TRANSFERINDO : String = 'C:\CC\HD\' ;
    CONFIG_DIR_AUDIO       : String = '' ;
    TERMO_FALANTE          : String = 'OK' ;
    FLAG_FINALIZAR         : Boolean = FALSE ;
    FLAG_MODO_AUTOMATICO   : Boolean = FALSE ;
    Log_Conversao          : TStringList ;
    Array_Diretorio        : Array [ 1 .. MAX_ARRAY_DIRETORIO ] Of Array Of T_Diretorio ;
    Array_CC               : Array Of T_Item_CC ;
    Last_Put_Log           : Cardinal = 0 ;
    PUT_LOG_ATIVO          : Boolean = TRUE ;
    MISC_ME_LOG_FFMPEG     : TMemo ;
    Misc_Log_TimeOut       : LongInt = 0 ;
    Misc_Log_Last          : String = '' ;
    Misc_Log_Indice        : LongInt = 0 ;
    Misc_Log_Cnt_Exception : LongInt = 0 ;
    Misc_Log_Cnt_Erros     : LongInt = 0 ;
    Misc_Log_Cnt_PushPop   : LongInt = 0 ;
    Misc_Log_Cnt_Atencao   : LongInt = 0 ;
    //Misc_LB_Log          : TListBox = Nil ;
    //Misc_LB_Log_Erros    : TListBox = Nil ;
    //Misc_LB_Log_Pesquisa : TListBox = Nil ;
    //Misc_CK_Rolagem_Log  : TCheckBox = Nil ;
    Misc_LB_Log_Chyron          : TListBox = Nil ;
//    Misc_LB_Log_Chyron_Erros    : TListBox = Nil ;
    Misc_CK_Rolagem_Chyron_Log  : TCheckBox = Nil ;
    PREVIEW_BMP            : TBitmap ;
    SL_HTTP                               : TStringList ;
    Tick_Proxima_Tentativa_HTTP           : Cardinal = 0 ;
    Indice_HTTP                           : Cardinal = 0 ;
    Setup_Servidor                        : LongInt = 123 ;
    MPX                                   : TWindowsMediaPlayer = Nil ;
    //PL_Mitok                              : TVLDSVideoPlayer = Nil ;
    TEXTO_PUT_LOG                         : String = '' ;
    UDP2                                  : T_UDP = Nil ;
    FLAG_CARREGANDO_SISTEMA_PUT_LOG       : Boolean = TRUE ;
    MISC_LB_Checar_Rede                   : TListBox = Nil  ;
    MISC_PN_Checar_Rede                   : TPanel = Nil ;
    //MISC_Audio_In                         : TALAudioIn ;
    //MISC_Audio_Out                        : TALAudioOut ;
    Misc_MemoryStatus                     : TMemoryStatusEx ;
...
Try
   ...
   End Else If Delimitador = #9 Then Begin
      ExtractStrings([#9],[], PChar(strLinha), Parte)
   End Else If Delimitador = #32 Then Begin
      ExtractStrings([#32],[], PChar(strLinha), Parte)
...
SL_PUT_DEBUG := TStringList.Create ;
SL_WindowsProcess := TStringList.Create ;
SetLength ( Tasks, 0 ) ;
...
SL_Erro_Get_Conteudo := TStringList.Create ;
Sys_Usuario_Aplicativo.Usuario := '-' ;
Sys_Usuario_Aplicativo.c_xLogin := Now ;
Lista_Query_Update := TStringList.Create ;
Lista_Query_Update.Clear ;
Dicionario_MM := TStringList.Create ;
SetLength ( A_Write_Delay, 0 ) ;
//
//SL_Blocos_CC2 := TStringList.Create ;
SL_Ancoras := TStringList.Create ;
//SL_Texto := TStringList.Create ;
_Ancoras_Espelho2 := TStringList.Create ;
SL_Usuarios                     := TStringList.Create ;
_Siglas                         := TStringList.Create ;
_Descricao                      := TStringList.Create ;
_Grupos_Carregados              := TStringList.Create ;
SL_Carregando_Sistema           := TStringList.Create ;
SL_Carregando_Sistema_Total     := TStringList.Create ;
Recusar_Gritos                  := TStringList.Create ;
_Jornais_Dominados              := TStringList.Create ;
Usuarios_Modificados            := TStringList.Create ;
_Espelhos_Carregados            := TStringList.Create ;
_Status_Criados                 := TStringList.Create ;
SL_View_Lista                   := TStringList.Create ;
_Palavras_Reservadas            := TStringList.Create ;
_Lista_Debug                    := TStringList.Create ;
Reporteres                      := TStringList.Create ;
SL_Atalhos                      := TStringList.Create ;
SL_Nomes_Usuarios_Zews          := TStringList.Create ;
SL_Categorias_Zews              := TStringList.Create ;
_Ancoras                        := TStringList.Create ;
Registros                       := TStringList.Create ;
Lista_Query_Update              := TStringList.Create ;
SL_Logs_Backup_Bruto            := TStringList.Create ;
SL_Debug_RR                     := TStringList.Create ;
SL_Usuarios_Codigo              := TStringList.Create ;
SL_Usuarios_Nome                := TStringList.Create ;
Lista_Logs                      := TStringList.Create ;
Lista_Relogio                   := TStringList.Create ;
Lista_Erro_Tipo_VT              := TStringList.Create ;
Lista_Lixeira                   := TStringList.Create ;
Relacao_ZAP                     := TStringList.Create ;
_Linhas_Pos                     := TStringList.Create ;
_FileName_Gritos                := TStringList.Create ;
Pautas_Previstas                := TStringList.Create ;
Lista_Geral                     := TStringList.Create ;
SL_Cercas_Recebidas_Key         := TStringList.Create ;
SL_Cercas_Recebidas_Lista       := TStringList.Create ;
SL_Matriz_Rastro                := TStringList.Create ;
SL_Rastro_Interativo            := TStringList.Create ;
SL_Clientes_Escopo              := TStringList.Create ;
Log_Ricardo                     := TStringList.Create ;
_Debug ( '1 INICIALIZACAO ULTRA_BASICA' ) ;
//For Local := 1 To MAX_ENTIDADES Do Begin
//   SL_Areas_Seguras[Local] := TStringList.Create ;
//End ;
//SL_Comando_Agendado             := TStringList.Create ;
//SL_Sites_Cliente                := TStringList.Create ;
SL_Buffer_Gravacao_Paginas_TCP  := TStringList.Create ;
SL_Buffer_Gravacao_Pacotes_xTCP := TStringList.Create ;
Lista_Violacao_Rota             := TStringList.Create ;
//SL_Total_Manutencao             := TStringList.Create ;
//SL_Violacao_Nova                := TStringList.Create ;
//SL_Linhas_ETUFOR                := TStringList.Create ;
SL_TCP_Connect                  := TStringList.Create ;
SL_Arquivos_Gerados             := TStringList.Create ;
SL_Monitoramento_Sistema        := TStringList.Create ;
SL_Handles_Conexoes_GPRS        := TStringList.Create ;
SL_Buffer_TCP_IP_HEX_GERAL      := TStringList.Create ;
SL_Buffer_TCP_IP_HEX_RDID       := TStringList.Create ;
SL_Buffer_TCP_IP_HEX_RDID_INV   := TStringList.Create ;
SL_Eventos_RD                   := TStringList.Create ;
//SL_Trajetos_Carregados          := TStringList.Create ;
SL_Status                       := TStringList.Create ;
//SL_Quebras_De_Rota_Console      := TStringList.Create ;
//SL_Quebras_de_Rota              := TStringList.Create ;
SL_Lista_FormCreate             := TStringList.Create ; // Text
SL_RPV                          := TStringList.Create ;
//SL_Clientes_Instalados          := TStringList.Create ;
SL_Comandos_RD_SQL              := TStringList.Create ;
SL_Rotas_Ida                    := TStringList.Create ;
SL_Rotas_Volta                  := TStringList.Create ;
// NW
Lista_Geral := TStringList.Create ;
SL_Carregando_Sistema := TStringList.Create ;
SL_Carregando_Sistema_Total := TStringList.Create ;
...
//Sys_DT_Carregando := Now ;
//Sys_Computador := COMPUTADOR ;
//Sys_Carga := Now ;
//Try
//  DT_Sys_Compilacao := FileDateToDateTime ( RR_FILE_AGE ( Application.ExeName ) ) ;
//  Except
//  DT_Sys_Compilacao := -1 ;
//  End ;
_Debug ( '3 INICIALIZACAO ULTRA_BASICA' ) ;
...
//GET_INFO_EXECUTAVEL ( Log , Resultado ) ;
...
Try
   CHECAR_DIRETORIO_FILENAME ( _FileName ) ;
   Lista_Geral.SaveToFile ( _FileName );
   Result := TRUE ;
Except
End ;
End ;
...
If RR_FileExists ( 'CARREGAR LISTA_GERAL', FN ) Then Begin
   Lista_Geral.Text := GET_CONTEUDO_M ( 'CARREGAR LISTA_GERAL', FN ) ;
End ;
End ;
...
End ;
{$IF NOT Defined(PROJETO_SETUP) AND NOT Defined(PROJETO_SERVICO)}
For Local := 0 To _SG.ColCount - 1 Do Begin
   MATRIZ_SG [ Local ] := _SG.Cells[Local,_SG.Row] ;
End ;
{$ENDIF}
Result := TRUE ;
...
If Assigned ( SL_Cercas_Recebidas_Key ) Then Begin
   SL_Cercas_Recebidas_Key.Free ;
End ;
_Debug ( '1 INI DESTROY_BASICO' ) ;
If Assigned ( SL_Matriz_Rastro ) Then Begin
   SL_Matriz_Rastro.Free ;
End ;
_Debug ( '2 DESTROY_BASICO' ) ;
{$IF NOT Defined(PROJETO_GPRS)}
Try
  If Assigned ( SL_Rastro_Interativo ) Then Begin
     SL_Rastro_Interativo.Free ;
  End ;
Except
End ;
{$ENDIF}
_Debug ( '3 DESTROY_BASICO' ) ;
If Assigned ( SL_Clientes_Escopo ) Then Begin
   SL_Clientes_Escopo.Free ;
End ;
If Assigned ( SL_Cercas_Recebidas_Lista ) Then Begin
   SL_Cercas_Recebidas_Lista.Free ;
End ;
//If Assigned ( Log_Cerca ) Then
//   Log_Cerca.Free ;
If Assigned ( Log_Ricardo ) Then Begin
   Log_Ricardo.Free ;
End ;
_Debug ( '4 DESTROY_BASICO' ) ;
Try
//   If RICARDO_REAL Then
//      ShowMessage ( 'destroy basico 1' ) ;
//   AGUARDE_SIMPLES ( 'Destroy Basico 1' ) ;
   _Debug ( '1 INI DESTROY_BASICO' ) ;
//   If Assigned ( Lista_TPR ) Then
//      Lista_TPR.Free ;
   {$IF Defined(PROJETO_CONSOLE)}
   For Local := 0 To MAX_ENTIDADES Do Begin
       With BUFFER_PUT_POSICAO_DDL [ Local ] Do Begin
   ...
   End ;
   {$ENDIF}
   _Debug ( '5 DESTROY_BASICO' ) ;
   //For Local := 1 To MAX_ENTIDADES Do Begin
   //   If Assigned ( SL_Areas_Seguras[Local] ) Then Begin
   //      SL_Areas_Seguras[Local].Free ;
   //   End ;
   //End ;
   //If Assigned ( SL_Comando_Agendado ) Then Begin
   //   SL_Comando_Agendado.Free ;
   //End ;
   //If Assigned ( SL_Sites_Cliente ) Then Begin
   //   SL_Sites_Cliente.Free ;
   //End ;
   _Debug ( '6 INI DESTROY_BASICO' ) ;
   {$IF Defined(PROJETO_ESQUINAS)}
   If Assigned ( SL_BAIRROS_GIS ) Then Begin
      SL_BAIRROS_GIS.Free ;
   End ;
   if Assigned ( SL_Rotas_Database ) Then Begin
      SL_Rotas_Database.Free ;
   End ;
   If Assigned ( SL_CIDADES_GIS ) Then Begin
      SL_CIDADES_GIS.Free ;
   End ;
   {$ENDIF}
   {$IF Defined(PROJETO_ESQUINAS)}
   If SL_LB_Roteiros <> Nil Then Begin
      SL_LB_Roteiros.Free ;
   End ;
   {$IFEND}
   If SL_Buffer_Gravacao_Pacotes_xTCP <> Nil Then Begin
      SL_Buffer_Gravacao_Pacotes_xTCP.Free ;
   End ;
   If Lista_Violacao_Rota <> Nil Then Begin
      Lista_Violacao_Rota.Free ;
   End ;
   //If Assigned ( SL_Total_Manutencao ) Then Begin
   //   SL_Total_Manutencao.Free ;
   //End ;
   //If SL_Violacao_Nova <> Nil Then Begin
   //   SL_Violacao_Nova.Free ;
   //End ;
   _Debug ( '7 DESTROY_BASICO' ) ;
   If SL_Buffer_Gravacao_Paginas_TCP <> Nil Then Begin
      SL_Buffer_Gravacao_Paginas_TCP.Free ;
   End ;
   {$IF NOT Defined(PROJETO_GPRS)}
   //If SL_Linhas_ETUFOR <> Nil Then Begin
   //   SL_Linhas_ETUFOR.Free ;
   //End ;
   {$ENDIF}
   If SL_TCP_Connect <> Nil Then Begin
      SL_TCP_Connect.Free ;
   End ;
   If SL_Arquivos_Gerados <> Nil Then Begin
      SL_Arquivos_Gerados.Free ;
   End ;
   {$IF NOT Defined(PROJETO_CONSOLE) AND NOT Defined(GPRS_MINIMO)}
   If Assigned ( SL_Monitoramento_Sistema ) Then Begin
      SL_Monitoramento_Sistema.Free ;
   End ;
   If Assigned ( SL_Handles_Conexoes_GPRS ) Then Begin
      SL_Handles_Conexoes_GPRS.Free ;
   End ;
   If Assigned ( SL_Buffer_TCP_IP_HEX_RDID_INV ) Then Begin
      SL_Buffer_TCP_IP_HEX_RDID_INV.Free ;
   End ;
   _Debug ( '8 DESTROY_BASICO' ) ;
   If Assigned ( SL_Buffer_TCP_IP_HEX_GERAL ) Then Begin
      SL_Buffer_TCP_IP_HEX_GERAL.Free ;
   End ;
   _Debug ( '9 INI DESTROY_BASICO' ) ;
   If Assigned ( SL_Buffer_TCP_IP_HEX_RDID ) Then Begin
      SL_Buffer_TCP_IP_HEX_RDID.Free ;
   End ;
   _Debug ( '10 INI DESTROY_BASICO' ) ;
   {$ENDIF}
   _Debug ( '11 INI DESTROY_BASICO' ) ;
   {
   If Assigned ( SL_RDID_REAL ) Then
      SL_RDID_REAL.Free ;
   }
   If Assigned ( SL_Eventos_RD ) Then Begin
      SL_Eventos_RD.Free ;
   End ;
   _Debug ( '12 INI DESTROY_BASICO' ) ;
   //If Assigned ( SL_Quebras_De_Rota_Console ) Then Begin
   //   SL_Quebras_De_Rota_Console.Free ;
   //End ;
   //If Assigned ( SL_Trajetos_Carregados ) Then Begin
   //   SL_Trajetos_Carregados.Free ;
   //End ;
   //If Assigned ( SL_Quebras_de_Rota ) Then Begin
   //   SL_Quebras_de_Rota.Free ;
   //End ;
   _Debug ( '13 INI DESTROY_BASICO' ) ;
   If Assigned ( SL_Lista_FormCreate ) Then Begin
      SL_Lista_FormCreate.Free ;
   End ;
   {
   If Assigned ( SL_Logotipos ) Then
      SL_Logotipos.Free ;
   }
   _Debug ( '14 INI DESTROY_BASICO' ) ;
   DESTROY_BASICO_INICIADO := TRUE ;
   _Debug ( '15 DESTROY_BASICO' ) ;
   If BOOT_OK Then Begin
      //
//      AGUARDE_SIMPLES ( 'Destroy Basico 2' ) ;
      _Debug ( '16 DESTROY_BASICO' ) ;
      {$IF Defined(PROJETO_CONSOLE) OR Defined(PROJETO_SEM_CONSOLE)}
      If Assigned ( SL_Conexoes_4001 ) Then Begin
         SL_Conexoes_4001.Free ;
      End ;
      If Assigned ( SL_Handles_4001 ) Then Begin
         SL_Handles_4001.Free ;
      End ;
      If Assigned ( SL_1st_Login_4001 ) Then Begin
         SL_1st_Login_4001.Free ;
      End ;
      If Assigned ( SL_DH_Conexoes_4001 ) Then Begin
         SL_DH_Conexoes_4001.Free ;
      End ;
      If Assigned ( SL_Cnt_Conexoes_4001 ) Then Begin
         SL_Cnt_Conexoes_4001.Free ;
      End ;
      {$IFEND}
      _Debug ( '17 DESTROY_BASICO' ) ;
      {$IFDEF THREADS}
      _Debug ( '18 DESTROY_BASICO' ) ;
//      AGUARDE_SIMPLES ( 'Destroy Basico 3 (Count=' + IntToStr ( SL_WriteFile_FileName.Count ) + ')' ) ;
      Tick := GetTickCount ;
      While ( GetTickCount - Tick ) < 30000 Do Begin
         If SL_WriteFile_FileName.Count = 0 Then Begin
            Break ;
         End ;
      End ;
      //
//      AGUARDE_SIMPLES ( 'Destroy Basico 3a (Count=' + IntToStr ( SL_Insert_SQL.Count ) + ')' ) ;
      Tick := GetTickCount ;
      While ( GetTickCount - Tick ) < 30000 Do Begin
         If SL_Insert_SQL.Count = 0 Then Begin
            Break ;
         End ;
      End ;
      //
//      AGUARDE_SIMPLES ( 'Destroy Basico 4 (Count=' + IntToStr ( SL_Append_File_FileName.Count ) + ')' ) ;
      _Debug ( '19 DESTROY_BASICO' ) ;
      Tick := GetTickCount ;
      While ( GetTickCount - Tick ) < 30000 Do Begin
         If SL_Append_File_FileName.Count = 0 Then Begin
            Break ;
         End ;
      End ;
      //
//      AGUARDE_SIMPLES ( 'Destroy Basico 4a (Count=' + IntToStr ( SL_Append_Linha_FileName.Count ) + ')' ) ;
      Tick := GetTickCount ;
      While ( GetTickCount - Tick ) < 30000 Do Begin
         If SL_Append_Linha_FileName.Count = 0 Then Begin
            Break ;
         End ;
      End ;
      //
//      AGUARDE_SIMPLES ( 'Destroy Basico 4b (Count=' + IntToStr ( SL_Zip_FileName.Count ) + ')' ) ;
      Tick := GetTickCount ;
      While ( GetTickCount - Tick ) < 30000 Do Begin
         If SL_Zip_FileName.Count = 0 Then Begin
            Break ;
         End ;
      End ;
      {$ENDIF}
      //
      {
      WRITE_FILE_REAL ( Diretorio + 'logs\writefile\' + DataHora_Inversa ( Now ) + '.dat' ,
                        SL_WriteFile_Log.Text ) ;
      }
   End ;
   {$IF Defined(PROJETO_CONSOLE) OR Defined(PROJETO_SEM_CONSOLE)}
   _Debug ( '20 DESTROY_BASICO' ) ;
   For Local := RDID_RASTREAMENTO_INI To RDID_RASTREAMENTO_FIM Do Begin
       If Assigned ( SL_Eventos_RX [Local] ) Then Begin
          SL_Eventos_RX [Local].Free ;
       End ;
   End ;
   {$IFEND}
//   AGUARDE_SIMPLES ( 'Destroy Basico 6' ) ;
   {
   If Assigned ( SL_Conversao_RDID_Reserva ) Then
      SL_Conversao_RDID_Reserva.Free ;
   }
   {$IF Defined(PROJETO_GPRS)}
//   AGUARDE_SIMPLES ( 'Destroy Basico 7' ) ;
   _Debug ( '21 DESTROY_BASICO' ) ;
   For Local := RDID_RASTREAMENTO_INI To RDID_RASTREAMENTO_FIM Do Begin
      If Assigned ( Banco_Dado_Lido[Local] ) Then Begin
         Banco_Dado_Lido[Local].Free ;
      End ;
   End ;
   {$IFEND}
//   AGUARDE_SIMPLES ( 'Destroy Basico 8' ) ;
   If Assigned ( SL_RPV ) Then Begin
      SL_RPV.Free ;
   End ;
//   AGUARDE_SIMPLES ( 'Destroy Basico 8a' ) ;
   //If Assigned ( SL_Clientes_Instalados ) Then Begin
   //   SL_Clientes_Instalados.Free ;
   //End ;
//   AGUARDE_SIMPLES ( 'Destroy Basico 8b' ) ;
   _Debug ( '22 DESTROY_BASICO' ) ;
   If Assigned ( SL_Comandos_RD_SQL ) Then Begin
      SL_Comandos_RD_SQL.Free ;
   End ;
   If Assigned ( SL_Rotas_Volta ) Then Begin
      SL_Rotas_Volta.Free ;
   End ;
   If Assigned ( SL_Rotas_Ida ) Then Begin
      SL_Rotas_Ida.Free ;
   End ;
   {$IFDEF THREADS}
//   AGUARDE_SIMPLES ( 'Destroy Basico 9' ) ;
   _Debug ( '10 DESTROY_BASICO' ) ;
   If Assigned ( SL_Append_File_FileName ) Then Begin
      SL_Append_File_FileName.Free ;
   End ;
   If Assigned ( SL_Append_File_Conteudo ) Then Begin
      SL_Append_File_Conteudo.Free ;
   End ;
   If Assigned ( SL_WriteFile_FileName ) Then Begin
      SL_WriteFile_FileName.Free ;
   End ;
//   AGUARDE_SIMPLES ( 'Destroy Basico 10' ) ;
   If Assigned ( SL_WriteFile_Conteudo ) Then Begin
      SL_WriteFile_Conteudo.Free ;
   End ;
   If Assigned ( SL_WriteFile_Log ) Then Begin
      SL_WriteFile_Log.Free ;
   End ;
   _Debug ( '23 DESTROY_BASICO' ) ;
   If Assigned ( SL_Insert_SQL ) Then Begin
      SL_Insert_SQL.Free ;
   End ;
   If Assigned ( SL_Append_Linha_FileName ) Then Begin
      SL_Append_Linha_FileName.Free ;
   End ;
//   AGUARDE_SIMPLES ( 'Destroy Basico 11' ) ;
   If Assigned ( SL_Zip_FileName ) Then Begin
      SL_Zip_FileName.Free ;
   End ;
   If Assigned ( SL_Zip_Arquivos ) Then Begin
      SL_Zip_Arquivos.Free ;
   End ;
   If Assigned ( SL_Append_Linha_Linha ) Then Begin
      SL_Append_Linha_Linha.Free ;
   End ;
//   AGUARDE_SIMPLES ( 'Destroy Basico 12' ) ;
   _Debug ( '24 DESTROY_BASICO' ) ;
   If Assigned ( SL_Grupos_Veiculos ) Then Begin
      SL_Grupos_Veiculos.Free ;
   End ;
//   AGUARDE_SIMPLES ( 'Destroy Basico 13' ) ;
//   _Debug ( '11 DESTROY_BASICO' ) ;
   {$ENDIF}
//   If Assigned ( SL_Veiculos_Extras ) Then Begin
//      SL_Veiculos_Extras.Free ;
//   End ;
   {
   If Assigned ( SL_Linhas_Debug_Parcial ) Then Begin
      SL_Linhas_Debug_Parcial.Free ;
   End ;
   If Assigned ( SL_Linhas_Debug ) Then Begin
      SL_Linhas_Debug.Free ;
   End ;
   }
...
For Local := 0 To Qtde - 1 Do Begin
   ...
   If Aux <> Aux_Old Then Begin
      ...
      End ;
      HistDic := HistDic + Dicionario_MM[Local] ;
      _Debug ( 'SUBST ' + IntToStr(Contador) + ' ' +
               'Key[' + Trim(Dicionario_MM[Local]) + '] ' +
               '[' + Aux_Old + ']==>[' + Aux + ']' ) ;
   End ;
End ;
