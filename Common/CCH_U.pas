...
If Trim ( CCH_Celulas[_Tag].Hist_Dic ) = '' Then Begin
   ShowMessage ( 'Nenhuma entrada do dicionário foi utilizada' ) ;
End Else Begin
   ShowMessage ( 'Aplicação do dicionário:' + sLineBreak +
                 sLineBreak +
   ...
   With CCH_Celulas[Local] Do Begin
      ...
      *)
      ...
      With Bot_Stop Do Begin
         ...
         ShowHint := TRUE ;
         Hint := 'Remover os efeitos não persistentes' ;
         Images := Main.IM_Main ;
         ImageMargins.Left := 5 ;
         ImageIndex := 44 ;
      End ;
      ...
      With Bot_AddStr Do Begin
         ...
         ShowHint := TRUE ;
         Hint := 'Adicionar #GC à lista de aceitação' ;
         Images := Main.IM_Main ;
         ImageMargins.Left := 5 ;
         ImageIndex := 30 ;
      End ;
      ...
      With Bot_Fixa Do Begin
         ...
         ShowHint := TRUE ;
         Hint := 'Reexibir última tarja fixa' ;
         Images := Main.IM_Main ;
         ImageMargins.Left := 5 ;
         ImageIndex := 45 ;
      End ;
      ...
      With Bot_Hist_Dic Do Begin
         ...
         ShowHint := TRUE ;
         Hint := 'Histórico de aplicação do dicionário' ;
         Images := Main.IM_Main ;
         ImageMargins.Left := 5 ;
         ImageIndex := -1 ;
      End ;
...
If ( Reg_Chyron.Msg = DEF_MSG_CHYRON_MANCHETE ) AND
   ...
   PUT_LOG ( 'Nenhuma ação (P1 ou P2 vazias) Msg[' + IntToStr(Reg_Chyron.Msg) + ']' ) ;
   // Não faz nada - alguma linha vazia
End Else If ( ( Reg_Chyron.Msg = DEF_MSG_CHYRON_GC ) OR
          ( Reg_Chyron.Msg = DEF_MSG_CHYRON_FIXA ) ) AND
        ( Trim  ( Reg_Chyron.P1 ) = '' ) Then Begin
   // Não faz nada - primeira linha vazia
   PUT_LOG ( 'Nenhuma ação (P1 vazia) Msg[' + IntToStr(Reg_Chyron.Msg) + ']' ) ;
