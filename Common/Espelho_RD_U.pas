...
Try
   ...
   RD_VAR_QTDE_RELOGIO_TEMPERATURA := 0 ; // Qtde de localidades no rodízio de temperatura
   ...
   RD_VAR_RETRANCAS_RESERVADAS.Add ( 'INÍCIO PREVIEW' ) ;
...
Try
   ...
   If NOT MyMessageDlg ( 'Confirma a exclusão da seção ?' ,
...
For Local := 1 To Qtde - 2 Do Begin
   ...
   If _Retranca = 'INÍCIO' Then Begin
...
Try
   ...
   With SG Do Begin
     ...
     Cells[SG_J_STATUS_EDICAO,0]  := 'Edição Texto' ;
...
PUT_STATUS ( 'Retranca selecionada para visualização [' + Registro.Retranca + ']' ) ;
...
If MAIN_GET_EDITOR_LAUDAS_VISIVEL Then Begin
   ShowNewMessage ( 'O editor de laudas já está aberto' + LFLF +
                    'Retranca "' + '' + '"' + LFLF +
                    'Conclua esta edição antes de abrir outra lauda' ) ;
...
If Pos ( '@' , Registro.Retranca ) > 0 Then Begin
   If NOT Sys_Usuario_Aplicativo.Sys_Modificar_Espelho Then Begin
      EXEC_Msg ( 'Seu nível de acesso não permite modificar espelhos.' ) ;
...
If TRUE Then Begin // NOT Escolhendo Then
   If ( Registro.Is_Comercial ) OR
      ...
      If NOT Sys_Usuario_Aplicativo.Sys_Fade Then Begin
         EXEC_Msg ( 'Estes dados poderão ser alterados somente ' +
                    'pela Coordenação.' ) ;
      End Else Begin
   ...
   If NOT Registro.Is_Especial Then Begin
      If NOT Registro.Is_Protegida Then Begin
         ...
         End ;
      End Else Begin
         RD_REGISTRA_LOG_ESPELHO ( 'Edição Negada Por Retranca Protegida "' +
                                   Registro.Retranca + '"' ) ;
         EXEC_Msg ( '"' + Registro.Retranca + '"' + sLineBreak +
                    sLineBreak +
                    'Esta retranca está protegida e não pode ser editada.' ) ;
      End
   End Else Begin
      RD_REGISTRA_LOG_ESPELHO ( 'Retranca Não Pode Ser Editada "' +
                                   Registro.Retranca + '"' ) ;
      EXEC_Msg ( '"' + Registro.Retranca + '"' + sLineBreak +
                 sLineBreak +
                 'Esta linha não pode ser editada.' ) ;
   End ;
End ; // If TRUE
_Debug ( 'FIM Espelho_RD SG_DUPLO_CLIQUE' ) ;
{$ENDIF}
...
Try
   ...
   End ;
   RD_VAR_TEMPO_RESTANTE_STR :=
       'Até o final >> Total: ' + SegundosToMMSS ( RD_VAR_RESTANTE_TT ) + '  =  ' +
       'Produção: ' + SegundosToMMSS ( RD_VAR_RESTANTE_PRODUCAO ) + ' + ' +
...
Try
   ...
   End Else Begin
      PUT_LOG ( 'NÃO OK Registro não atualizado (Idx ' + IntToStr(Result) + ') ' +
                   '[' + Registro.Retranca + ']' ) ;
   End ;
Except
   On E : Exception Do Begin
...
Try
   ...
   For Local := 0 To RD_VAR_QTDE_RETRANCAS_SAVE - 1 Do
      ...
      If RD_VAR_Array_RETRANCAS_SAVE[Local].Retranca =
         ...
         End ;
      End ;
   If Result = -1 Then Begin // Retranca a ser salva não encontrada no espelho atualizado
...
Try
   ...
   If Posicao = -1 Then Begin
      PUT_LOG ( 'Exit RD_RENOMEAR_RETRANCA Não Encontrado no Espelho' ) ;
   ...
   If Ok Then Begin
      For Local := 0 To Qtde - 1 Do Begin
         ...
         End Else Begin
            PUT_LOG ( 'RENAME NÃO OK de [' + Lista1[Local] +
                      '] para [' + Lista2[Local] + ']' ) ;
            Sucesso := FALSE ;
         End ;
      End ;
   ...
   For Local := 0 To Qtde - 1 Do Begin
      ...
      If CopiaArquivo ( 'TEspelho_RD.RD_CRIAR_BACKUP_RETRANCA_EXCLUIDA',
         ...
         PUT_LOG ( 'RD_CRIAR_BACKUP_RETRANCA_EXCLUIDA OK ' +
                   '[' + Origem + '][' + Destino + ']' ) ;
      End Else Begin
         PUT_LOG ( 'RD_CRIAR_BACKUP_RETRANCA_EXCLUIDA NÃO OK ' +
                   '[' + Origem + '][' + Destino + ']' ) ;
      End ;
   End ;
   Lista.Free ;
...
Try
   ...
   If RD_VAR_ESPELHO_ATUAL.Selecionada.Is_Especial Then Begin
      ShowNewMessage ( 'Esta linha não pode ser removida do espelho.' ) ;
      _Debug ( 'Exit Is_Especial Espelho_RD EXCLUIR_RETRANCA' ) ;
      Exit ;
   End ;
   If NOT MyMessageDlg ( 'Confirma a exclusão da retranca "' + Retranca + '" ?' ,
                         mtInformation, [mbYes, mbNo],
                         ['Sim','Não'], 'Confirmação', Resposta_Dialog ) Then Begin
      ShowNewMessage ( 'A operaçao de exclusão foi cancelada.' ) ;
      _Debug ( 'Exit Not Confirm Espelho_RD EXCLUIR_RETRANCA' ) ;
      Exit ;
   End ;
   Indice := RD_GET_INDICE_RETRANCA ( RD_BUFFER_MAIN, Retranca ) ;
   If Indice = -1 Then Begin
      ShowNewMessage ( 'A retranca não foi encontrada no espelho.' ) ;
   ...
   For Local := 0 To Qtde - 1 Do Begin
      ...
      End Else Begin
         PUT_LOG ( 'RD_EXCLUIR_RETRANCA NÃO OK [' + Lista[Local] + ']' ) ;
      End ;
   End ;
   Lista.Free ;
...
End Else If Tag_Origem = -120 Then Begin
   RD_SET_TIPO_SECAO ( 'RD_TRATAR_DRAGDROP,Tag:' + IntToStr(Tag_Origem),
                       Tag_Destino, RD_TIPO_GC, RELOAD_TRUE ) ;
   GC_LIMPAR_PREVIEW ;
   SELECIONAR_TARJA_VIDEOGRAFISMO ( 'DragDrop/Botão', Tag_Destino, '',
                                    Cod, -1 ) ;
End Else Begin
   ...
   For Local := 0 To RD_VAR_QTDE_RETRANCAS - 1 Do Begin
      ...
      With RD_VAR_ARRAY_RETRANCAS[Local] Do Begin
         ...
         End Else Begin
            PN.Color := clSilver ;
            PN.Font.Color := clGray ;
            PN.Caption := 'RETRANCA NÃO MODIFICADA (' + _Retranca + ')' ;
         End ;
         Exit ;
      End ;
   End ;
...
If NOT RR_FileExists ( 'RD LOAD_BROWSERS/2', RD_GET_FN_BROWSERS ) Then Begin
   ShowMessage ( 'Browsers do Chyronhego não foram carregados.' + sLineBreak +
                 'Serão utilizadas as configurações realizadas diretamente no console do Chyron' ) ;
   Exit ;
End ;
Try
...
If NOT RR_FileExists ( 'RD LOAD_DIRETORIOS_MESSAGES/2',
                       RD_GET_FN_DIRETORIOS_MESSAGES ) Then Begin
   ShowMessage ( 'Diretórios Messages do Chyronhego não foram carregados' + sLineBreak +
                 'Serão utilizadas as configurações realizadas diretamente no console do Chyron' ) ;
   Exit ;
End ;
Try
...
If NOT RR_FileExists ( 'RD VERIFICAR_FN_IPS_CAMERA_TRANSITO',
   ...
   Str := Str + '192.168.0.5/' + '=' + 'Av. ANTÔNIO SALES|Fortaleza' + sLineBreak ;
...
While Resultado = 0 Do Begin
   ...
   End Else Begin
      PUT_LOG ( 'DeleteFile NÃO OK [' + RD_GET_DIR_JORNAL_LOCAL + RecDos.Name + ']' ) ;
   End ;
   Resultado := System.SysUtils.FindNext ( RecDos ) ;
End ;
System.SysUtils.FindClose ( RecDos ) ;
...
If RD_VAR_TASK_SYNC_MODO_THREAD Then Begin
   If Assigned ( RD_VAR_TASK_SYNC_JORNAL ) Then Begin
      If RD_VAR_TASK_SYNC_JORNAL.Status = TTaskStatus.Running Then Begin
         PUT_LOG ( 'Task RD SINCRONIZAR_JORNAL_COMPLETO em execução' ) ;
         Exit ;
      End ;
   End ;
