...
If Assigned ( Sup.Task_Process ) Then Begin
   ...
   If Sup.Task_Process.Status = TTaskStatus.Running Then Begin
      ...
      PUT_LOG ( 'Task Process em execução' ) ;
      Exit ;
   End ;
End ;
...
Result := 'Quantidade de devices de vídeo ativos encontrados: ' +
          IntToStr(Qtde) + sLineBreak +
          sLineBreak ;
Result := Result + 'Pixels para formulário: ' +
          ...
          'Usuário: [' + NOME_USUARIO + ']' + sLineBreak +
          sLineBreak ;
...
For Local := 0 To Qtde - 1 Do
   Begin
   Result := Result +
             'Device de vídeo ' + IntToStr(Local+1) + sLineBreak +
             ...
             'Índice: ' + IntToStr ( Screen.Monitors[Local].MonitorNum ) + sLineBreak +
             'Primário: ' + IFSN ( Screen.Monitors[Local].Primary ) + sLineBreak +
             sLineBreak ;
   End ;
...
Try
   ...
   While oEnum.Next(1, FWbemObject, iValue) = 0 Do Begin
      ...
      Result := Result + 'Informações sobre o adaptador de vídeo ' + IntToStr(Contador) + sLineBreak ;
      If NOT VarIsNull ( FWbemObject.Name ) Then
...
Except
   On E : Exception Do Begin
      Result := 'Sem informações válidas de GetWin32_VideoControllerInfo' + sLineBreak  ;
      Result := 'Exceção: [' + E.Message + ']' + sLineBreak  ;
...
While(Check) Do
   ...
   Result := Result +
             'Configuração de vídeo ' + IntToStr(ModeNumber) + ': ' +
...
Sup.Sup_ME_Drives.Lines.Add ( 'Drives lógicos (Qtde=' + IntToStr(Length(Sup.Sup_Drives)) + '):' ) ;
...
If Assigned ( Sup.Task_Drives ) Then Begin
   If Sup.Task_Drives.Status = TTaskStatus.Running Then Begin
      PUT_LOG ( 'Task Drives em execução' ) ;
      Exit ;
   End ;
End ;
